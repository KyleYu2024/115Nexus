...
<body>
    ...
    <div class="container">
        ...
        <div id="tab-settings" class="tab-content">
            ...
            <div style="text-align:center; color:var(--text-sub); font-size:12px; margin-bottom:30px;">Version v0.2.0</div>
        </div>
    </div>
    ...
    <script>
        ...
        async function openResources(id,type,title){document.getElementById('resModal').style.display='flex';document.getElementById('resTitle').innerText=decodeURIComponent(title);document.getElementById('resList').innerHTML='<div style="text-align:center;padding:30px;">⏳</div>';try{const r=await fetch('/api/resources?id='+id+'&type='+type);const d=await r.json();if(!d.items||!d.items.length){document.getElementById('resList').innerHTML='<div style="text-align:center;padding:30px;">📭</div>';return;}document.getElementById('resList').innerHTML=d.items.map(res=>{let tags=(res.tags||[]).map(t=>'<span class="tag '+(t.includes('4K')?'tag-4k':'')+'">'+t+'</span>').join('');return '<div class="res-item"><div class="res-top"><div class="res-title">'+res.display+'</div><button class="btn btn-sm '+(res.hdhive_points>0?'btn-blue':'btn-green')+'" onclick="pushResource(this,\''+res.link+'\','+res.hdhive_points+')">转存</button></div><div style="margin-top:5px;">'+tags+'</div></div>';}).join('');}catch(e){document.getElementById('resList').innerHTML='失败';}}
